...
print(f'Heart, speed {SPEED}; device={device}, threads={torch.get_num_threads()}, temperature supervisor={GUARDED}. {completed_words:,}/{total_words:,} words already saved.', flush=True)
...
metadata = [';FFMETADATA1', 'title=Emotional Design — Heart', 'artist=Donald A. Norman', 'album=Emotional Design', 'genre=Audiobook', 'comment=AI-generated narration: Kokoro Heart, American English, speed 0.95. Includes EPUB text and figure captions. Refer to the EPUB for images.']
elapsed_ms = 0
for chapter in chapters:
...
cover = Path.home() / 'Library/Mobile Documents/iCloud~com~apple~iBooks/Documents/Emotional Design.epub/OEBPS/cover.jpg'
output = ROOT / 'Emotional Design — Heart.m4b'
temporary = ROOT / 'Emotional Design — Heart.partial.m4b'
print('Combining the audiobook and checking all audio.', flush=True)
run(['ffmpeg', '-v', 'error', '-y', '-f', 'concat', '-safe', '0', '-i', WORK / 'concat.txt', '-i', WORK / 'metadata.txt', '-i', cover, '-map', '0:a', '-map', '2:v', '-map_metadata', '1', '-map_chapters', '1', '-c', 'copy', '-disposition:v', 'attached_pic', '-metadata', 'media_type=2', '-movflags', '+faststart', temporary])
